...
import {
  ArrowRight, Search, Star, Users, Download,
...
import {
  ...
  SiApple, SiSpotify, SiWhatsapp, SiVk, SiNaver,
  SiSteam, SiEpicgames, SiNetflix,
} from 'react-icons/si';
...
const TRUST_ICONS = [ShieldCheck, Cloud, Zap, BadgeCheck, Download];

const FLOATING_ICONS = [
  { id: 'instagram', icon: SiInstagram, size: 96, wrap: 'top-6 right-16', rotate: -8, delay: 0.1, duration: 6.2, z: 40, opacity: 1, scale: 1.02 },
  { id: 'facebook', icon: SiFacebook, size: 88, wrap: 'top-20 right-28', rotate: 6, delay: 0.4, duration: 5.8, z: 30, opacity: 0.95, scale: 1 },
  { id: 'gmail', icon: SiGmail, size: 84, wrap: 'top-36 right-8', rotate: -5, delay: 0.7, duration: 6.6, z: 35, opacity: 0.98, scale: 1.03 },
  { id: 'tiktok', icon: SiTiktok, size: 90, wrap: 'top-16 left-[40%]', rotate: 12, delay: 0.3, duration: 6.9, z: 20, opacity: 0.9, scale: 0.98 },
  { id: 'telegram', icon: SiTelegram, size: 82, wrap: 'top-[42%] right-[24%]', rotate: -6, delay: 0.5, duration: 5.9, z: 25, opacity: 0.95, scale: 1.01 },
  { id: 'youtube', icon: SiYoutube, size: 100, wrap: 'top-[52%] right-10', rotate: 5, delay: 1.1, duration: 6.4, z: 35, opacity: 1, scale: 1.05 },
  { id: 'x', icon: SiX, size: 86, wrap: 'top-[62%] left-[42%]', rotate: -4, delay: 0.8, duration: 6.5, z: 22, opacity: 0.9, scale: 0.98 },
  { id: 'pinterest', icon: SiPinterest, size: 80, wrap: 'top-[72%] right-[10%]', rotate: 8, delay: 1.4, duration: 6.8, z: 18, opacity: 0.88, scale: 0.96 },
  { id: 'google', icon: SiGoogle, size: 88, wrap: 'top-[80%] left-[34%]', rotate: -3, delay: 1.2, duration: 7.1, z: 20, opacity: 0.92, scale: 0.99 },
  { id: 'discord', icon: SiDiscord, size: 92, wrap: 'bottom-[12%] right-[18%]', rotate: 7, delay: 0.9, duration: 5.7, z: 28, opacity: 0.97, scale: 1.02 },
  { id: 'reddit', icon: SiReddit, size: 86, wrap: 'bottom-24 left-[46%]', rotate: -6, delay: 1.3, duration: 6.3, z: 16, opacity: 0.86, scale: 0.97 },
  { id: 'apple', icon: SiApple, size: 88, wrap: 'bottom-20 left-16', rotate: 5, delay: 1.6, duration: 7.0, z: 14, opacity: 0.8, scale: 0.95 },
  { id: 'spotify', icon: SiSpotify, size: 94, wrap: 'top-[12%] right-[10%]', rotate: -5, delay: 2.4, duration: 7.2, z: 36, opacity: 1, scale: 1.04 },
  { id: 'whatsapp', icon: SiWhatsapp, size: 100, wrap: 'bottom-[28%] right-[6%]', rotate: 6, delay: 2.0, duration: 7.2, z: 36, opacity: 1, scale: 1.04 },
  { id: 'vk', icon: SiVk, size: 82, wrap: 'bottom-[38%] left-[26%]', rotate: -6, delay: 2.6, duration: 6.8, z: 16, opacity: 0.87, scale: 0.95 },
  { id: 'naver', icon: SiNaver, size: 80, wrap: 'bottom-[10%] left-[26%]', rotate: 5, delay: 2.8, duration: 6.2, z: 14, opacity: 0.84, scale: 0.96 },
  { id: 'steam', icon: SiSteam, size: 90, wrap: 'top-[8%] left-[10%]', rotate: -4, delay: 3.0, duration: 6.6, z: 24, opacity: 0.95, scale: 1.0 },
  { id: 'epic-games', icon: SiEpicgames, size: 88, wrap: 'bottom-[16%] left-[18%]', rotate: 7, delay: 3.2, duration: 6.9, z: 18, opacity: 0.88, scale: 0.98 },
  { id: 'netflix', icon: SiNetflix, size: 86, wrap: 'top-[34%] right-[18%]', rotate: 6, delay: 3.6, duration: 6.4, z: 22, opacity: 0.9, scale: 0.98 },
];

const TrustBadge = ({ text, compact = false }) => {
  if (!text) return null;
  return (
    <motion.div
      initial="hidden"
...
const HeroSearchBar = ({ className = '', size = 'lg', placeholder = '' }) => {
...
const CategoryPills = ({ className = '', scrollable = false, label = 'Popular' }) => {
  ...
  return (
    <div className={`flex items-center gap-2 ${scrollable ? 'overflow-x-auto no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0' : 'flex-wrap'} ${className}`}>
      <span className="text-xs font-semibold text-muted-foreground/80 shrink-0">{label}:</span>
      {pills.map((tag) => (
        <Link
...
const CTAButtons = ({
  ...
  return (
    <div className={`flex ${stacked ? 'flex-col' : 'flex-wrap'} gap-3 ${className}`}>
      ...
      {secondaryText && (
        <Link
          to={secondaryTo || '/become-a-seller'}
  ...
  return (
    <div className={`flex flex-wrap items-center gap-x-6 gap-y-2 ${className}`}>
      {items.map((item, index) => {
        ...
        return (
          <div key={item.id || label} className="flex items-center gap-1.5 text-xs font-medium text-foreground/70">
    ...
    >
      <dl className={`grid ${cols === 3 ? 'grid-cols-3' : 'grid-cols-2 lg:grid-cols-4'} divide-x divide-border/70 text-center`}>
        {stats.map((s) => {
          ...
          return (
            <div key={s.id || s.label} className="flex flex-col items-center justify-center gap-1 sm:gap-1.5 px-1 sm:px-2">
...
const DesktopMarketplaceVisual = ({ backgroundImage }) => {
  ...
  if (backgroundImage) {
    return (
      <div className="relative w-full h-[32rem] xl:h-[36rem] rounded-[2rem] overflow-hidden">
...
const HeroSection = () => {
  ...
  const { data: slidesDoc } = useCms(CMS_KEYS.HERO_SLIDES);
  const { data: global } = useCms(CMS_KEYS.GLOBAL);
  const { data: popularSection } = useCms(CMS_KEYS.POPULAR_TAGS);

  const hero = useMemo(() => {
    const sections = Array.isArray(homepage?.sections) ? homepage.sections : [];
    return sections.find((s) => s.key === 'hero' || s.type === 'hero') || null;
  }, [homepage]);

  const activeSlide = useMemo(() => {
    const slides = Array.isArray(slidesDoc?.items) ? slidesDoc.items : [];
    return [...slides]
      .filter((s) => s && (s.status === 'active' || s.status === 'published' || !s.status))
  ...
  const secondaryText = hero?.secondaryButtonText || '';
  const secondaryTo = hero?.secondaryButtonUrl || '/become-a-seller';
  const backgroundImage = hero?.backgroundImage || activeSlide?.backgroundImage || '';
  const brand = global?.siteName || '';
  const trustItems = Array.isArray(hero?.trustItems) ? hero.trustItems : [];
  const heroStats = Array.isArray(homepage?.heroStats) ? homepage.heroStats : [];
  const popularLabel = useMemo(() => {
    const sections = Array.isArray(homepage?.sections) ? homepage.sections : [];
    const tagsSection = sections.find((s) => s.key === 'popular_tags' || s.type === 'popular_tags');
    return tagsSection?.title || 'Popular';
  }, [homepage]);

  // Suppress unused warning — popular tags loaded via CategoryPills; keep doc warm.
  void popularSection;

  const bgStyle = backgroundImage
    ? { backgroundImage: `linear-gradient(135deg, rgba(243,238,255,0.92), rgba(251,247,255,0.88)), url(${backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }
    : undefined;

  return (
    <section
      className="mx-auto max-w-[90rem] px-3 sm:px-5 lg:px-8 pt-4 lg:pt-6"
      aria-label={brand ? `Welcome to ${brand}` : 'Welcome'}
    >
      <div
        className="relative overflow-hidden rounded-[2rem] lg:rounded-[2.5rem] bg-gradient-to-br from-[#F3EEFF] via-[#FBF7FF] to-[#FFEFFA] border border-white/70 soft-shadow"
        style={bgStyle}
      >
          ...
          <div className="grid grid-cols-2 gap-14 items-center">
            <div>
              ...
              {(title || brand) && (
                ...
                >
                  {brand && <span className="brand-text">{brand}</span>}
                  {brand && title ? ' — ' : ''}
                  {title}
                </motion.h1>
              )}
              ...
              {subtitle && !description && (
                <motion.p initial="hidden" animate="show" custom={2} variants={fadeUp} className="mt-5 text-lg text-muted-foreground max-w-lg leading-relaxed">
                  {subtitle}
                </motion.p>
              )}
              {description && (
              ...
              {searchPlaceholder && (
                <motion.div initial="hidden" animate="show" custom={3} variants={fadeUp} className="mt-7">
                  <HeroSearchBar placeholder={searchPlaceholder} />
                </motion.div>
              )}

              <motion.div initial="hidden" animate="show" custom={4} variants={fadeUp} className="mt-4">
                <CategoryPills label={popularLabel} />
              </motion.div>

              <motion.div initial="hidden" animate="show" custom={5} variants={fadeUp} className="mt-8">
                <CTAButtons
                  primaryTo={primaryTo}
              ...
              <motion.div initial="hidden" animate="show" custom={6} variants={fadeUp}>
                <TrustStrip items={trustItems} className="mt-7" />
              </motion.div>
            </div>

            <div className="hidden lg:block">
              <DesktopMarketplaceVisual backgroundImage={backgroundImage} />
            </div>
          </div>

          <StatsRow stats={heroStats} cols={4} className="mt-12" />
        </div>

        <div className="lg:hidden relative px-4 sm:px-6 pt-8 pb-8">
          <TrustBadge text={badgeText} compact />
          ...
          {description && (
            <motion.p initial="hidden" animate="show" custom={2} variants={fadeUp} className="mt-3 text-sm sm:text-[15px] text-muted-foreground leading-relaxed max-w-md">
              {description}
            </motion.p>
          )}
          ...
          {searchPlaceholder && (
            <motion.div initial="hidden" animate="show" custom={3} variants={fadeUp} className="mt-5">
              <HeroSearchBar size="sm" placeholder={searchPlaceholder} />
            </motion.div>
          )}

          <motion.div initial="hidden" animate="show" custom={4} variants={fadeUp} className="mt-3.5">
            <CategoryPills scrollable label={popularLabel} />
          </motion.div>
          ...
          <StatsRow stats={heroStats.slice(0, 3)} cols={3} className="mt-7" />
        </div>
      </div>
    </section>
  );

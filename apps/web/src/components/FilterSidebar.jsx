...
import { SlidersHorizontal, Star, RotateCcw, ShieldCheck, Sparkles, PackageCheck } from 'lucide-react';
import {
  PRICE_RANGES,
  RATING_FILTERS,
  FILE_TYPE_FILTERS,
...
const FilterSidebar = ({ filters, onChange, categorySlot, resultCount, showSellerFilter = true }) => {
  const set = (patch) => onChange({ ...filters, ...patch });
  const toggleIn = (key, value) => set({ [key]: filters[key].includes(value) ? filters[key].filter((v) => v !== value) : [...filters[key], value] });

  const isDefault = filters.price === DEFAULT_FILTERS.price
    && filters.rating === DEFAULT_FILTERS.rating
    && filters.fileTypes.length === 0
    && filters.licenses.length === 0
    && (filters.deliveryTime || 'any') === 'any'
    ...
    <div className="rounded-[1.5rem] border border-border bg-white p-6 shadow-sm lg:sticky lg:top-[9.5rem]">
      <div className="mb-5 flex items-center justify-between">
        ...
        {!isDefault && (
          ...
          >
            <RotateCcw className="h-3 w-3" aria-hidden="true" /> Clear
          </button>
        )}
      </div>
      ...
      <Group title="Availability">{AVAILABILITY_FILTERS.map((r) => <Radio key={r.id} name="availability" checked={(filters.availability || 'any') === r.id} onChange={() => set({ availability: r.id })}><span className="inline-flex items-center gap-1">{r.id === 'in_stock' && <PackageCheck className="h-3.5 w-3.5 text-primary" aria-hidden="true" />}{r.label}</span></Radio>)}</Group>
      <Group title="Delivery">{DELIVERY_TIME_FILTERS.map((r) => <Radio key={r.id} name="deliveryTime" checked={(filters.deliveryTime || 'any') === r.id} onChange={() => set({ deliveryTime: r.id })}>{r.label}</Radio>)}</Group>
      ...
      {showSellerFilter && (
        <Group title="Seller">
          <Check checked={!!filters.verifiedOnly} onChange={() => set({ verifiedOnly: !filters.verifiedOnly })}>
            <span className="inline-flex items-center gap-1"><ShieldCheck className="h-3.5 w-3.5 text-primary" /> Verified sellers only</span>
          </Check>
        </Group>
      )}
    </div>

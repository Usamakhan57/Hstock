...
const PLATFORM_KEYWORDS = [
  // Contact apps — mentioning them in chat is treated as off-platform intent
  ...
  // Marketplace product brands — only block profile/share URLs, not status discussion
  // (e.g. "Instagram disabled" / "Facebook checkpoint" must remain allowed)
  { rule: 'instagram', patterns: [/instagram\.com\//i, /instagr\.am\//i] },
  { rule: 'facebook', patterns: [/facebook\.com\//i, /\bfb\.com\//i] },
  { rule: 'tiktok', patterns: [/tiktok\.com\//i, /vm\.tiktok\.com/i] },
  { rule: 'twitter', patterns: [/twitter\.com\//i, /(?:^|[^\w])x\.com\//i] },
  { rule: 'snapchat', patterns: [/snapchat\.com\//i] },
...
/**
 * OCR evidence scan — narrower than chat text filter.
 * Account UI screenshots often contain words like "Facebook", "Instagram",
 * "Gmail", "cPanel". Those must NOT auto-flag.
 *
 * Flag only clear contact / off-platform transfer signals:
 * phones, emails, Telegram/WhatsApp links, external URLs, wallet addresses.
 */
export function detectOcrSensitiveContent(raw) {
  const rules = [];
  const { originalLower, collapsed } = normalizeForDetection(raw);
  if (!originalLower && !collapsed) {
    return { blocked: false, rules, sensitive: false };
  }
  ...
  if (
    /\b(?:t\.me|telegram\.me)\b/i.test(originalLower)
    || /\btg\s*:/i.test(originalLower)
    || /\bwa\.me\b/i.test(originalLower)
    || /chat\.whatsapp\.com/i.test(originalLower)
    || /discord\.gg/i.test(originalLower)
    || /discord\.com\/invite/i.test(originalLower)
  ) {
    pushRule(rules, 'off_platform_link');
  }

  const digitsOnly = originalLower.replace(/\D/g, '');
  if (
    /(?:\+|00)?\d{1,3}[\s-]?\d{2,4}[\s-]?\d{3,4}[\s-]?\d{3,4}/.test(originalLower)
    || (digitsOnly.length >= 10 && /(?:number|phone|mobile|whatsapp|call|sms|\+)/i.test(originalLower))
    || (digitsOnly.length >= 11 && /(?:\+|00)/.test(originalLower))
  ) {
  ...
  if (
    /\b[13][a-km-zA-HJ-NP-Z1-9]{25,34}\b/.test(raw)
    || /\bbc1[a-z0-9]{25,87}\b/i.test(originalLower)
    || /\b0x[a-f0-9]{40}\b/i.test(originalLower)
    || /\bT[1-9A-HJ-NP-Za-km-z]{33}\b/.test(raw)
  ) {
    pushRule(rules, 'wallet_address');
  }

  // Explicit invite-to-leave-platform phrases inside screenshot text
  if (
  ...
  return { blocked: rules.length > 0, rules, sensitive: rules.length > 0 };
}

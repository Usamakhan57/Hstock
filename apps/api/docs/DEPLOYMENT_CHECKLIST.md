# Deployment Checklist — ApnaStore v1.0 RC
...
- [ ] Domain DNS → VPS (`apnastore.org`, `www`)
...
- [ ] Clone release tag/branch to `/var/www/apnastore`
...
- [ ] `npm run build` (web) → sync `dist/` to `/var/www/apnastore/web`
...
- [ ] Nginx site from `deploy/nginx/apnastore.conf`
- [ ] Certbot SSL issued and HTTP→HTTPS redirect active
- [ ] Socket.io proxied at `/socket.io/`
- [ ] Logrotate installed from `deploy/logrotate/apnastore`
...
- [ ] `pm2 monit` / `pm2 logs apnastore-api`
- [ ] Disk / memory / CPU alerts configured (Hostinger or Node exporter)
- [ ] Backup archive appears under `/var/backups/apnastore`
